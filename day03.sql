...
-- Drop table tablo_adi'ndan sonra CASCADE komutunu kullanırız

-- Talebeler tablosundaki isim sütununa NOT NULL kısıtlaması ekleyiniz 
...
--Tablodaki bir sutüna Primary Key ekleme
...
add constraint fk_id foreign key (id) REFERENCES ogrenciler;
-- Child tabloyu parent tablodan oluşturduğumuz için sütun adı verilmedi

-- PK'yı silme constraint silme
alter table ogrenciler
drop CONSTRAINT pk_id
--FK'yi silme CONTRAINT silme
alter table ogrenci_adres DROP CONSTRAINT fk_id;

-- Yazılı notu 85 den buyuk talebe bilgilerini getirin
...
Bir tanesi gerçekleşmezse listelemez
    Select * from matematik where sinav1 > 50 AND sınav2 > 50
Hem sınav1 hemde sınav2 alanı, 50'den büyük olan kayıtları listeler
    ...
    select * From matematik where sınav>50 OR sınav2>50 
...
-- maaşı 70000 veya ismi Sena olan peesoneli listele
...
--isiminin 2. harfi e olan personeli listele
